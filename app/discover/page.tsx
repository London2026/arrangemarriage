...
import { isTrialActive, trialDaysLeft as calcTrialDaysLeft, TRIAL_LIMITS } from '@/lib/trial'
...
export default async function DiscoverPage() {
  ...
  const [{ count: meetingsCount }, { count: extraPurchased }, { count: likesCount }, { count: revealsCount }] = await Promise.all([
    ...
    supabase.from('profile_likes').select('*', { count: 'exact', head: true })
      .eq('liker_id', user.id).gte('created_at', countSince),
    revealsQ,
  ])
  ...
  let revealsTotal: number  // -1 = unlimited (paid), 0 = expired trial, 5 = active trial
